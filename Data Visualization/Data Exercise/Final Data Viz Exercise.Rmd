...
- [Microsoft Excel](https://drive.google.com/file/d/196FfUaUGOm4gWbOi_K7VR_y3wmOPC0PM/view?usp=sharing) (download the spreadsheet)
...
- (Optional) What is the range, median, and mean for displacement (`disp`), year, # of 
...
Bar Chart: 
1. Choose a model, make a bar chart that shows both the `cty` and `hwy` mpg over 
time
2. Does that model have multiple `cyl` options? If so, plot those in separate 
panels (aka facets)

Scatter Plot:
1. Filter the data for 2008 only; make a scatter plot that has displacement 
(`displ`) on the y-axis and highway gas mileage (`hwy`) on the x-axis.
...
ggplot(plot_dat, aes(x = hwy, y = displ)) +
    geom_point(color = "#1a3f6f") +
    ggtitle("Engine Displacement vs. Highway MPG in 2008") +
    xlab("Highway (mpg)") +
...
### 4. Review
- Does anything stick out in your bar charts? How about in the scatter plot?
